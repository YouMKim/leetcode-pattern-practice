...
export const POINTERS_TRICKS = [
  ...
  {
    id: 'pointers/at-most-k',
    name: 'Exactly-K = atMost(K) − atMost(K−1)',
    when: 'COUNT subarrays with exactly K distinct/odd/whatever — "exactly" windows don’t slide, "at most" ones do',
    code: [
      'def at_most(k):',
      '    count = left = 0',
      ...
      '        freq[x] += 1',
      '        while len(freq) > k:            # shrink to validity',
      '            freq[nums[left]] -= 1',
      '            if freq[nums[left]] == 0:',
      '                del freq[nums[left]]',
      '            left += 1',
    ...
    bigO: { time: 'O(n) per at_most pass', space: 'O(k)' },
    gotchas: [
      '"Exactly K" windows aren’t monotone — shrinking can stay valid, so direct sliding fails',
      'count += window length: every left end within the window makes a valid subarray ending here',
      'Works for K distinct, K odd numbers, sum ≤ K, and every "count the subarrays" cousin',
    ],
    quiz: [
      {
        q: 'Why can’t a single sliding window count "exactly K distinct" directly?',
        options: [
          'Windows can’t count',
          'The exactly-K property isn’t monotone under shrinking — the window bounds aren’t well-defined',
          'It can, in O(n)',
          'defaultdict is too slow',
        ],
        answer: 1,
        why: 'At-most-K IS monotone; the subtraction converts the answer.',
      },
    ],
    problems: [
      { title: 'Subarrays with K Different Integers', diff: 'H', slug: 'subarrays-with-k-different-integers' },
      { title: 'Binary Subarrays With Sum', diff: 'M', slug: 'binary-subarrays-with-sum' },
      { title: 'Count Number of Nice Subarrays', diff: 'M', slug: 'count-number-of-nice-subarrays' },
    ],
  },
];
